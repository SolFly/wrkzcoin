...
void LmDBWrapper::setDataDir(const DataBaseConfig &config)
{
    ...
    {
        m_dbDir = config.getDataDir() + '/' + TESTNET_DB_NAME;
    }
    else
    {
        m_dbDir = config.getDataDir() + '/' + DB_NAME;
    }
    m_dbFile = m_dbDir + "/data.mdb";
}

std::string LmDBWrapper::getDataDir(const DataBaseConfig &config)
{
    return m_dbDir;
}
